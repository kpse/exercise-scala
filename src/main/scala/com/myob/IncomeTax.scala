...
case class IncomeTax(taxTable: List[String]) {
  ...
  def of(salary: Double): Option[RoundingDollar] = {
    taxRangeList.find(_.isInRange(salary)).map(_.amount(salary)).map(RoundingDollar.apply)
  }

  def taxRangeList = {
    //"$18,201 - $37,000       19c for each $1 over $18,200"
    val range =
      """^\$*(\d+)\s*-\s*\$(\d+)\s+(\d*\.?\d*)c for each \$1 over \$(\d+)$""".r
    //"$37,001 - $80,000       $3,572 plus 32.5c for each $1 over $37,000"
    val rangeWithBasic =
      """^\$*(\d+)\s*-\s*\$(\d+)\s+\$(\d+) plus (\d*\.?\d*)c for each \$1 over \$(\d+)$""".r
    //0 - $18,200     Nil
    val rangeBelowTaxable =
      """^\$*(\d+)\s*-\s*\$(\d+)\s+Nil\s*$""".r
